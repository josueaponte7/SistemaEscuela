...
$campos['campos'] = 'ps.id_programa,  ps.nombre_programa';
$resultado        = $obj->getPrograma($campos);
$total            = $obj->totalFilas('programa_social', 'id_programa',$campos['condicion']);
...
// titulo del listado
$titulo = "LISTADO DE PROGRAMAS SOCIALES";
$pdf->Ln(5);
$pdf->SetX(60);
...
$pdf->Cell($w_idprogarma, $row_height, 'ID', 1, 0, 'C', 1);
$pdf->Cell($w_programa, $row_height, 'Programa Social', 1, 1, 'L', 1);
...
for ($i = 0; $i < count($resultado); $i++) {
    ...
    $nombre_programa = $resultado[$i]['nombre_programa'];
    ...
    $pdf->SetFont('FreeSerif', '', 12);
    $pdf->SetX(50);
    $pdf->Cell($w_idprogarma, $row_height, $id_programa, 1, 0, 'C', 1);
    $pdf->Cell($w_programa, $row_height, $nombre_programa, 1, 1, 'L', 1);
    $j++;
}
...
$pdf->Output('listado_programaSocial.pdf', 'I');
