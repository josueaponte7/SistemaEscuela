...
$obj              = new Estado();
$campos['campos'] = 'id_estado,nombre_estado';
$resultado        = $obj->getEstados($campos);
$total            = $obj->totalFilas('estado','id_estado',$campos['condicion']);
...
$pdf->Cell($w_idestado, $row_height, 'ID', 1, 0, 'C', 1);
$pdf->Cell($w_estado, $row_height, 'Estado', 1, 1, 'L', 1);
...
for ($i = 0; $i < count($resultado); $i++) {
    ...
    $id_estado     = $resultado[$i]['id_estado'];
    ...
    $pdf->SetFont('FreeSerif', '', 12);
    $pdf->SetX(50);
    $pdf->Cell($w_idestado, $row_height, $id_estado, 1, 0, 'C', 1);
    $pdf->Cell($w_estado, $row_height, $nombre_estado, 1, 1, 'L', 1);
    $j++;
}
