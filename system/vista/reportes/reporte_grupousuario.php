...
$campos['campos'] = 'gu.id_grupo,  gu.nombre_grupo';
$resultado        = $obj->getGrupo($campos);
$total            = $obj->totalFilas('grupo_usuario','id_grupo',$campos['condicion']);
...
// titulo del listado
$titulo = "LISTADO DE GRUPO USUARIOS";
$pdf->Ln(5);
$pdf->SetX(60);
...
$pdf->Cell($w_idgrupo, $row_height, 'ID', 1, 0, 'C', 1);
$pdf->Cell($w_nombre, $row_height, 'Grupo Usuarios', 1, 1, 'L', 1);
...
for ($i = 0; $i < count($resultado); $i++) {
    ...
    $id_grupo     = $resultado[$i]['id_grupo'];
    $nombre_grupo = $resultado[$i]['nombre_grupo'];
    ...
    $pdf->SetFont('FreeSerif', '', 12);
    $pdf->SetX(50);
    $pdf->Cell($w_idgrupo, $row_height, $id_grupo, 1, 0, 'C', 1);
    $pdf->Cell($w_nombre, $row_height, $nombre_grupo, 1, 1, 'L', 1);
    $j++;
}
...
$pdf->Output('listado_grupoUsuarios.pdf', 'I');
