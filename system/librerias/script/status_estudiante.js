$(document).ready(function() {
    var TStatusestu = $('#tabla_statusestu').dataTable({
        "iDisplayLength": 5,
        "iDisplayStart": 0,
    ...
    $('#registrar').click(function() {
        $('#registro_statusestu').slideDown(2000);
        $('#reporte_statusestu').slideUp(2000);
    });

    $('#todos').change(function() {
        var TotalRow = TStatusestu.fnGetData().length;
        var nodes = TStatusestu.fnGetNodes();
        if (TotalRow > 0) {
            if ($(this).is(':checked')) {
    ...
    $('table#tabla_statusestu').on('change', 'input:checkbox[name="id_estatus[]"]', function() {
    ...
    $('#imprimir').click(function() {
        var url = '../reportes/reporte_statusestudiante.php';
        if ($('#todos').is(':checked')) {
            url = url + '?todos=1';
    ...
    $('#guardar').click(function() {
        ...
        if ($('#nombre').val() === null || $('#nombre').val().length === 0 || /^\s+$/.test($('#nombre').val())) {
            $('#div_statusestudi').addClass('has-error');
            $('#nombre').focus();
        } else {
            ...
            if ($(this).text() == 'Guardar') {
                ...
                var TotalRow = TStatusestu.fnGetData().length;
                if (TotalRow > 0) {
                    var lastRow = TStatusestu.fnGetData(TotalRow - 1);
                ...
                $($id_estatus).prependTo($('#frmstatus_estudiante'));

                $.post("../../controlador/StatusEstudiante.php", $("#frmstatus_estudiante").serialize(), function(respuesta) {
                    if (respuesta == 1) {

                        window.parent.bootbox.alert("Registro con Exito", function() {
                            // Agregar los datos a la tabla
                            TStatusestu.fnAddData([$check_status, codigo, $('#nombre').val(), modificar, eliminar]);
                            $('input[type="text"]').val('');
                        });
                    }
                });
                            ...
                            $.post("../../controlador/StatusEstudiante.php", $("#frmstatus_estudiante").serialize(), function(respuesta) {
                                if (respuesta == 1) {
                                    ...
                                    window.parent.bootbox.alert("Modificacion con Exito", function() {
                                        ...
                                        $("#tabla_statusestu tbody tr:eq(" + fila + ")").find("td:eq(2)").html($('#nombre').val());
                                        $('input[type="text"]').val('');
    ...
    $('#salir').click(function() {
        ...
        $('#registro_statusestu').slideUp(2000);
        $('#reporte_statusestu').slideDown(2000);
        $('#id_estatus').remove();
        $('input:text').val('');
    });
    ...
    $('table#tabla_statusestu').on('click', 'img.modificar', function() {
        $('#id_estatus').remove();
        ...
        $($fila).prependTo($('#frmstatus_estudiante'));

        var $id_estatus = '<input type="hidden" id="id_estatus"  value="' + id_estatus + '" name="id_estatus">';
        $($id_estatus).appendTo($('#frmstatus_estudiante'));
    });

    // modificar las funciones de eliminar
    $('table#tabla_statusestu').on('click', 'img.eliminar', function() {
        $('#id_estatus').remove();
        var padre = $(this).closest('tr');
        ...
        window.parent.bootbox.confirm({
            ...
            callback: function(result) {
                if (result) {
                    var id_estatus = padre.children('td:eq(1)').text();
                    $.post("../../controlador/StatusEstudiante.php", {'accion': 'Eliminar', 'id_estatus': id_estatus}, function(respuesta) {
                        if (respuesta == 1) {

                            window.parent.bootbox.alert("Eliminacion con Exito", function() {
                                //borra la fila de la tabla
                                TStatusestu.fnDeleteRow(nRow);
